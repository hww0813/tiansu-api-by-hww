...
/**
 * Created by xucan on 2019-11-20 14:41
 */
public class ExcelToHtmlUtils {
    ...
    /**
     * 程序入口方法
     * @param sourcefile 要转换的excel
     * @param isWithStyle 是否需要表格样式 包含 字体 颜色 边框 对齐方式
     * @return <table>...</table> 字符串
     */
    public static File readExcelToHtml(File sourcefile , boolean isWithStyle) throws Exception {
        ...
        try {
            ...
            if (wb instanceof XSSFWorkbook) {
                ...
                htmlExcel = ExcelToHtmlUtils.getExcelInfo(xWb,isWithStyle);
            }else if(wb instanceof HSSFWorkbook){
                HSSFWorkbook hWb = (HSSFWorkbook) wb;
                htmlExcel = ExcelToHtmlUtils.getExcelInfo(hWb,isWithStyle);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }finally{
        ...
        return generateFile(sourcefile,htmlExcel);
    }

    //生成文件
    private static File generateFile(File sourcefile, String htmlExcel) throws Exception {
        File file = null;
        String path = sourcefile.getParentFile().getAbsolutePath();
        String sfName = sourcefile.getName();
        String fileName = sfName.substring(0,sfName.lastIndexOf("."))+".html";
